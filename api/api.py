...
def list_image_urls():
    ...
    images = ['/media/' + x.name for x in Path(unlabeled_folder).iterdir(
...
def serve_image_url(filename):
    ...
    orig_images_path = Path(os.environ['IMAGES_DIRECTORY'])
    unlabeled_folder = os.path.join(
        str(orig_images_path), 'swipe_labeler_data', 'unlabeled')
...
def submit_label():
    ...
    positive_folder = os.path.join(
        str(orig_images_path), 'swipe_labeler_data', 'labeled_positive')
    pos_path_string = positive_folder + image_name

    negative_folder = os.path.join(
        str(orig_images_path), 'swipe_labeler_data', 'labeled_negative')
